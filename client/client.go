...
func (client *Client) HandleCommand(cmd Command) error {
	// Write command to client connection
	// Wait for receipt of error
	// If no error return nil
	wg.Add(1)
	c := make(chan Message)
	switch cmd.Cmd {
	case "/nick":
		command := cmd.Cmd + cmd.Params[1]
		client.Write(client.Conn, command, false)
		go client.Read(client.Conn, c)

		msgRcv := <-c

		fmt.Println(msgRcv.Channel)

		if len(cmd.Params) < 1 {
			return errors.New("(461) Need more parameters: /nick <nickname>")
		}

	}

	return errors.New("(433) Nickname is already in use")

}

func (client *Client) Read(conn io.ReadWriter, c chan Message) {
	s := bufio.NewScanner(conn)
	if s.Scan() {
		line := s.Text()
		log.Println(line)
		msgRcv := ParseMessage(line, client)
